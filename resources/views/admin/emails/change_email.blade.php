...
<body>
    ...
    <br/>
    <a href="{{ $url }}">{{ $url }}</a><br/><br/><br/>本URLの有効期限は発行より{{ config('auth.email_auth_timeout')/60 }}分となっております。<br>
    期限が切れてしまった際には、再度ログイン画面よりメールアドレスの変更を行なってください。
    <br/><br/><br/>
    ...
    OPNiD 問い合わせ窓口<br>
    <a href="https://{{ config('app.subdomain_admin') }}/iquiry">
        https://{{ config('app.subdomain_admin') }}/iquiry
    </a>
    <br/><br/>OPNiD<br>
    <a href="https://{{ config('app.subdomain_admin') }}">
        https://{{ config('app.subdomain_admin') }}
    </a>
</body>
</html>
